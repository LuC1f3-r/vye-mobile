...
export default function AddStateScreen() {
  ...
  // Scroll-aware save button
  const [contentOverflows, setContentOverflows] = useState(false);
  const saveButtonTranslateY = useSharedValue(100); // starts hidden below
  ...
  const onContentSizeChange = useCallback((w: number, h: number) => {
    setContentHeight(h);
    const overflows = h > scrollViewHeight + SCROLL_THRESHOLD;
    setContentOverflows(overflows);
    if (!overflows) {
      // Content fits on screen, always show save button
      saveButtonTranslateY.value = withTiming(0, { duration: 300 });
    }
  }, [scrollViewHeight, saveButtonTranslateY]);

  const handleScroll = useCallback((event: NativeSyntheticEvent<NativeScrollEvent>) => {
    if (!contentOverflows) return;
    const offsetY = event.nativeEvent.contentOffset.y;
    if (offsetY > SCROLL_THRESHOLD) {
      saveButtonTranslateY.value = withTiming(0, { duration: 250 });
    } else {
      saveButtonTranslateY.value = withTiming(100, { duration: 250 });
    }
  }, [contentOverflows, saveButtonTranslateY]);

  const saveButtonAnimatedStyle = useAnimatedStyle(() => ({
    transform: [{ translateY: saveButtonTranslateY.value }],
    opacity: saveButtonTranslateY.value > 50 ? 0 : 1,
  }));
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]} edges={['top', 'bottom']}>
      ...
      >
        ...
        {/* Save Button with scroll-aware animation */}
        <Animated.View
          style={[styles.saveContainer, { backgroundColor: colors.background }, saveButtonAnimatedStyle]}
        >
          <TouchableOpacity
