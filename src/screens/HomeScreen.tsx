...
const isMarkedDay = (day: number | null) => {
  return day !== null && (PERIOD_DAYS.includes(day) || FERTILE_DAYS.includes(day));
};
...
export default function HomeScreen() {
  ...
  const renderCalendarRow = (row: (number | null)[], rowIndex: number) => {
    // Check if band continues into next/previous rows
    const prevRow = rowIndex > 0 ? calendarRows[rowIndex - 1] : null;
    const nextRow = rowIndex < calendarRows.length - 1 ? calendarRows[rowIndex + 1] : null;
    ...
    return (
      <View key={rowIndex} style={styles.calendarRow}>
        {row.map((day, colIndex) => {
          ...
          const inBand = inFertile || inPeriod;

          // Same-row neighbors
          const prevDay = colIndex > 0 ? row[colIndex - 1] : null;
          const nextDay = colIndex < 6 ? row[colIndex + 1] : null;
          const prevInBand = prevDay !== null && isMarkedDay(prevDay);
          const nextInBand = nextDay !== null && isMarkedDay(nextDay);

          // Cross-row continuity: check if band wraps from the end of prev row or to the start of next row
          const continuesFromPrevRow = colIndex === 0 && prevRow && isMarkedDay(prevRow[6]);
          const continuesToNextRow = colIndex === 6 && nextRow && isMarkedDay(nextRow[0]);

          const isFirstInBand = inBand && !prevInBand && !continuesFromPrevRow;
          const isLastInBand = inBand && !nextInBand && !continuesToNextRow;
